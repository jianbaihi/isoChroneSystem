...
async def create_name_cloud(
    ...
    if request.profile != "foot-walking":
        raise InvalidProviderParameterError("profile", "name_cloud_requires_foot_walking")
    if tuple(request.rangesMinutes) != (10, 20, 30):
        raise InvalidProviderParameterError("rangesMinutes", "name_cloud_requires_10_20_30")
    if len(request.cumulativeIsochrones) != 3 or [item.rangeMinutes for item in request.cumulativeIsochrones] != [10, 20, 30]:
        raise InvalidProviderParameterError("cumulativeIsochrones", "name_cloud_requires_10_20_30")

    outer_geometry = shape(request.cumulativeIsochrones[-1].geometry)
    area_km2 = geodesic_area_km2(outer_geometry)
    maximum_area = 45.0
    if area_km2 > maximum_area:
        raise PoiCoverageAreaExceededError(area_km2, maximum_area)
    ...
    return AnalysisResult(
        ...
        metadata=AnalysisMetadata(
            ...
            requestId=request_id,
            warnings=[
                "等时圈复用当前成功的真实 ORS 步行结果。",
                "POI 使用最外层 30 分钟真实 Polygon 单次查询。",
