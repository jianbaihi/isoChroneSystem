...
class OrsRemotePoiProvider:
    ...
    def validate_request(self, request: AnalysisRequest) -> None:
        ...
        if not request.rangesMinutes:
            raise InvalidProviderParameterError("rangesMinutes", "must_not_be_empty")
    ...
    async def fetch(
        ...
        *,
        ...
        upstream_limit_hit = bool(not single_polygon and initial_cell_count > len(cells))

        async def query_cell(cell: PoiCell, depth: int = 0) -> None:
            nonlocal request_count, cache_hits, rate_limit, api_quota, upstream_limit_hit
            if request_count >= self.settings.ors_poi_max_requests_per_analysis:
                upstream_limit_hit = True
                return
            request_count += 1
        ...
        return {
            ...
            "coverage": {
                ...
                "fullyCovered": not bool(upstream_limit_hit),
