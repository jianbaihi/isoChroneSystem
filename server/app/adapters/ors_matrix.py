...
def matrix_band_id(duration_seconds: float, ranges_minutes: list[int] | tuple[int, ...] = (10, 20, 30)) -> str:
    previous = 0
    for current in ranges_minutes:
        if duration_seconds <= current * 60:
...
class OrsMatrixAdapter:
    ...
    def parse_response(
        ...
        *,
        ...
        ranges_minutes: list[int] | tuple[int, ...] = (10, 20, 30),
        ...
        for index, poi in enumerate(pois):
            ...
            if raw_duration is None or raw_distance is None:
                ...
                status = "ok"
            matrix_band = matrix_band_id(duration, ranges_minutes) if status == "ok" and duration is not None else None
